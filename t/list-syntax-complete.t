...
subtest 'Empty list [] syntax support' => sub {
    ...
    is( $result, 'AI::Logic::List->new()',
        'Empty array [] converts to empty List constructor'
    );

    # Test through _make_arg_list
    my @args = AI::Logic::Database::_make_arg_list( [] );
    is( $args[0], 'AI::Logic::List->new()',
        '_make_arg_list handles empty list syntax'
    );
};

subtest 'Empty list singleton' => sub {
    my $list1 = AI::Logic::List->new();
    my $list2 = AI::Logic::List->new();
    is( $list1, $list2, 'Empty list instances are identical' );
};
...
# Should create nested List constructors: List('a', List('b', List('c', List())))
    ...
    is( scalar(@args), 1,
        '_make_arg_list returns single argument for array'
    );
    like(
        $args[0], qr/AI::Logic::List->new\('x'/,
        '_make_arg_list processes list elements'
    );
};
...
subtest 'List syntax with variables' => sub {
    ...
    my @args = AI::Logic::Database::_make_arg_list(
        [ $named_var, 'atom', $any_var ] );
    like(
        $args[0], qr/\{PACKAGE\}::X/,
        'Named variable in list handled correctly'
    );
    like( $args[0], qr/'atom'/, 'Atom in list handled correctly' );
    like(
        $args[0], qr/\{PACKAGE\}::Any\(\)/,
        'Any variable in list handled correctly'
    );
};
...
subtest 'Nested list syntax support' => sub {
    ...
    like(
        $result, qr/AI::Logic::List->new\(AI::Logic::List->new/,
        'Nested list structure created'
    );
...
subtest 'Head|Tail pattern conceptual support' => sub {
  ...
  # Note: This is conceptual since Perl doesn't have native [Head|Tail] syntax
   ...
   # by supporting variables and nested structures in lists
    ...
    like(
        $args[0], qr/\{PACKAGE\}::Head/,
        'Head variable in list supported'
    );
...
# Test that list syntax integrates properly with existing _make_arg_list functionality
    ...
    my @args
      = AI::Logic::Database::_make_arg_list( $var, $any, $list, $atom );
